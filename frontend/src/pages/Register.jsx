...
import { RiFilmLine, RiUserLine, RiMailLine, RiLockLine, RiEyeLine, RiEyeOffLine } from 'react-icons/ri'
...
const Register = () => {
  ...
  useEffect(() => {
    document.title = 'Register - FlickBrain'
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col justify-center items-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#13131a] rounded-xl border-t-4 border-t-[#7c3aed] border-x border-b border-[#1e1e2e] shadow-2xl p-8 z-10">
        <div className="mb-6 text-center">
          ...
          <p className="text-[#94a3b8] text-sm">Join FlickBrain today and unlock your personalized engine.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Name field */}
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-slate-300 flex items-center gap-1.5" htmlFor="name">
              <RiUserLine className="w-4 h-4 text-[#94a3b8]" />
              <span>Full Name</span>
            </label>
            <input
              id="name"
              type="text"
              className={`input-field ${errors.name ? 'border-rose-500 focus:border-rose-500 focus:ring-rose-500/20' : ''}`}
            ...
            />
            {errors.name && (
              <p className="text-xs text-rose-500 font-medium mt-0.5">{errors.name}</p>
            )}
          </div>
